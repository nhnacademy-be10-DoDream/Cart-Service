...
//		Long cartItemId = 1L;
//
//		mockMvc.perform(delete("/carts/{cartId}/cart-items/{cartItemId}", 1L,cartItemId))
...
//		given(cartItemService.getBookByIdForItem(item)).willReturn(book);
//
//		mockMvc.perform(get("/carts/{cartId}/cart-items/{cartItemId}",item.getCartId() ,item.getCartItemId()))
//				.andExpect(status().isOk())
//				.andExpect(jsonPath("$.bookTitle").value("Book A"))
