...
class CartItemServiceTest {
	...
	@Test
	@DisplayName("removeCartItem: 특정 아이템을 성공적으로 삭제한다")
	void removeCartItem_success() {
		...
		// then
		verify(cartItemRepository).existsById(cartItemId);
		verify(cartItemRepository).deleteById(cartItemId);
	}
	
	@Test
	@DisplayName("removeCartItem: 삭제할 아이템이 없으면 예외를 던진다")
	void removeCartItem_notFound_throwsException() {
		// given
		given(cartItemRepository.existsById(cartItemId)).willReturn(false);
		
		// when & then
		assertThatThrownBy(() -> cartItemService.removeCartItem(cartItemId))
				.isInstanceOf(DataNotFoundException.class);
